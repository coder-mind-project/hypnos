...
class SMTPMail implements Mail.Options {
    ...
    public validate(): void {
        if (!this.from)
            throw new InvalidArgument("Emissor de e-mail deve ser informado");

        if (!this.to)
            throw new InvalidArgument("Receptor de e-mail deve ser informado");

        if (!this.text && !this.html)
            throw new InvalidArgument("Conteúdo de e-mail deve ser informado");
    }
}
